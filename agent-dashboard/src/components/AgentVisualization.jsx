...
const AgentVisualization = ({ interactions, agentColors, runningAgents, onEdgeClick, highlightedStep }) => {
  ...
  return (
    ...
    >
      ...
      {Object.values(agents).map((agent) => (
        ...
        >
          <div
            className="w-24 h-24 rounded-full flex items-center justify-center text-white font-bold shadow-lg relative"
            style={{ backgroundColor: agent.color }}
          >
            <span className="text-xs text-center">{agent.id}</span>

            {/* Spinning gear for running agents */}
            {runningAgents && runningAgents[agent.id] && (
